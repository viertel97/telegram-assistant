...
async def get_bookmark_transcriptions(xml_data, caption, update: Update):
    ...
    for file_name, group in df.groupby("fileName"):
        await log_to_telegram(
            "start downloading and processing of file: " + file_name, logger, update
        )
        ...
        logger.info(
            "downloaded file '{file_name}' - start conversion".format(
                file_name=file_name
            )
        )
        sound = AudioSegment.from_file(file_name)
        logger.info(
            "converted file '{file_name}' - start reading and transcriptions".format(
                file_name=file_name
            )
        )
        for row_index, row in group.iterrows():
            ...
            logger.info(
                "extracting audio segment from file: "
                + file_name
                + " at position: "
                + str(file_position)
            )
            if file_position < 5:
                ...
                temp_sound = sound[
                    (file_position - 5) * 1000 : (file_position + 5) * 1000
                ]

            temp_file_name = "{file_name}-{file_position}.mp3".format(
                file_name=file_name[:-4], file_position=str(file_position)
            )
            ...
            logger.info(
                "uploaded file '{file_name}' to todoist".format(
            ...
            logger.info(
                "transcribing audio segment from file: "
                + file_name
                + " at position: "
                + str(file_position)
                + " in de-DE & en-US"
            ...
            if "title" in row.keys() and row["title"] is not None:
                result_timestamp = datetime.strptime(
                    row["title"], "%Y-%m-%dT%H:%M:%S%z"
                )
            else:
    ...
    message = "finished processing {} files from {} by {} and extracted {} transcriptions".format(
        len(df), title, author, len(result_list)
    )
