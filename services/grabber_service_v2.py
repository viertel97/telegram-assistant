...
from services.grabber_service import (
...
async def dump_todoist_to_monica(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        "Dump was done at {timestamp}".format(timestamp=timestamp)
    )
...
def get_grabber_data(days_to_dump):
    ...
    list_of_files = [
        create_index_file_from_dict(
            index_file_name, timestamp, df_items[df_items["parent_id"].isnull()]
        )
    ]
...
def handle_offset(data):
    is_default_offset, offset, gmt_string = get_default_offset_including_check(
        DEFAULT_OFFSET
    )
    if not is_default_offset:
        ...
        data[row_indexer] = (
            pd.to_datetime(data["created_at"]) + pd.Timedelta(offset)
...
def get_comments(days):
    ...
    df_notes.rename(
        columns={"item_id": "parent_id", "posted_at": "created_at"}, inplace=True
    )
...
def get_items(days, df_projects, df_labels, df_notes):
    df_items = pd.concat(
        [
            ...
            clean_completed_tasks(
                get_completed_tasks(datetime.today() - timedelta(days=days + 1))
            ),
        ],
        ignore_index=True,
    )
    df_items = df_items.merge(
        df_projects[["project_id", "project"]], on="project_id", how="left"
    )
    ...
    df_filtered_items = df_filtered_items[
        df_filtered_items["parent_id"].isin(df_filtered_items["id"])
        | df_filtered_items["parent_id"].isnull()
    ]

    df_filtered_items["created_at"] = pd.to_datetime(
        df_filtered_items["created_at"]
    ) + pd.Timedelta("01:00:00")
    df_filtered_items["created_at_string"] = df_filtered_items[
        "created_at"
    ...
    df_filtered_items["content"] = df_filtered_items["content"].replace(
        {"Ş": "S", "ş": "s"}, regex=True
    )
...
def get_data(days):
    df_projects = clean_api_response(TODOIST_API.get_projects()).rename(
        columns={"id": "project_id", "name": "project"}
    )
    ...
    df_items[["slugified_title"]] = (
        df_items["summary"].apply(lambda x: slugify(x)).apply(pd.Series)
    )
...
def create_position_in_hierarchy(df_items):
    ...
    for parent_id, group in grouped:
        ...
        for i in range(len(sorted_group)):
            ...
            item_id = item.name
            df_items.at[item_id, "prev_item"] = (
                id_to_content[prev_item.name] if prev_item is not None else None
            )
            df_items.at[item_id, "next_item"] = (
                id_to_content[next_item.name] if next_item is not None else None
            )
    ...
    df_items["child_contents"] = df_items.index.map(
        lambda x: [id_to_content[child_id] for child_id in child_ids_dict[x]]
        if x in child_ids_dict
        else None
    )

    df_items["parent_content"] = df_items["parent_id"].map(
        lambda x: id_to_content[x] if x in id_to_content else None
    )
    return df_items
...
def generate_front_matter(
    ...
    metadata_json = {
        ...
        "down": [f"[[{title}]]" for title in down_element_titles]
        if down_element_titles
        else [],
