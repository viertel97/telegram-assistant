...
CLOUDFLARE_API_KEY, CLOUDFLARE_ZONE_ID, PRIVATE_IP, HOSTED_ID = get_secrets(
    ["cloudflare/api_key", "cloudflare/zone_id", "cloudflare/private_ip", "cloudflare/hosted_ip"])
...
def get_current_dns_entries():
    r = requests.get(f'https://api.cloudflare.com/client/v4/zones/{CLOUDFLARE_ZONE_ID}/dns_records', headers=HEADERS)
    result = r.json()
...
async def toggle_ips(update: Update, context: CallbackContext):
    ...
    for dns_entry in current_dns_entries:
        temp = requests.put(
            ...
            headers=HEADERS,
            json={"name": dns_entry["name"], "type": dns_entry["type"], "content": selected_ip, "ttl": 1,
                  "proxied": proxied})
