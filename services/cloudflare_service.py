...
async def toggle_ips(update: Update, context: CallbackContext):
    ...
    if HOSTED_ID in str(current_dns_entries) and PRIVATE_IP not in str(
        current_dns_entries
    ):
        await log_to_telegram(
            "switch from non_private_ip to private_ip", logger, update
        )
        selected_ip = PRIVATE_IP
        proxied = False
    else:
        await log_to_telegram(
            "switch from private_ip to non_private_ip", logger, update
        )
    ...
    for dns_entry in current_dns_entries:
        ...
        if temp.status_code != 200:
            await log_to_telegram(
                f"error while switching ips: {temp.json()}", logger, update
            )
        else:
