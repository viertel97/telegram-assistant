...
from services.notion_service import stretch_project_tasks, stretch_article_list
...
async def stretch_TPT(update: Update, context: CallbackContext):
    await stretch_project_tasks(update, context)


async def stretch_Articles(update: Update, context: CallbackContext):
    await stretch_article_list(update, context)
