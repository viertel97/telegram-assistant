...
def get_filtered_findings(findings):
    ...
    if proportions.head(1).values[0] > FINDING_THRESHOLD:
        return [
            ...
            if page["page"].number == proportions.head(1).index[0]
        ], page_numbers
...
async def handle_pdf(file_path, file_name, update: Update):
    ...
    tasks = [
        ...
        for tasks in get_items_by_todoist_project(project.id)
    ]
    ...
    if selected_book_from_todoist in grouped_df.groups:
        ...
        for _, task in group.iterrows():
            ...
            if len(findings) and not text_to_search == "" and not text_to_search == " ":
                ...
                if filtered_findings:
                    try:
                        content = await add_annotation_to_finding(
                            filtered_findings, task, text_to_search
                        )
                        command_list.append(
                            {"type": "item_close", "args": {"id": task.id}}
                        )
                        ...
                        await log_to_telegram(
                            "Error while highlighting text '{}' on page {}.".format(
                    ...
                    await log_to_telegram(
                        "The text '{}' was found on different pages [{}] and can't be highlighted".format(
                            text_to_search, str(page_numbers)
                        ),
                        logger,
                        update,
                    )
                ...
                await log_to_telegram(
                    "nothing found for '{}'".format(text_to_search), logger, update
                )
                not_found_counter += 1
        if not_found_counter == len(group):
            await log_to_telegram(
                "nothing found for all {} tasks".format(len(group)), logger, update
            )
            return
        new_file_path = file_path[:-4].replace("_V" + str(file_version), "")
        new_file_path = os.path.join(
            new_file_path + "_V" + str(file_version + 1) + ".pdf"
        )
        ...
        await update.message.reply_document(
            ...
            caption="'{}' out of possible '{}' could not have been added - therefore '{}' annotations has been added sucecessfully to the PDF '{}'.".format(
                not_found_counter,
                group.shape[0],
                group.shape[0] - not_found_counter,
                file_name,
            ),
        )
        ...
        if response.status_code != 200:
            ...
            await log_to_telegram(
                "synced todoist with positive response", logger, update
            )
        ...
        await log_to_telegram(
            "no book for the caption '{}' found".format(selected_book_from_todoist),
            logger,
            update,
        )
...
async def add_annotation_to_finding(filtered_findings, task, text_to_search):
    ...
    if task.annotation:
        ...
        content = (
            f"timestamp: {task.recording_timestamp}\ntext_to_search: {text_to_search}"
        )
