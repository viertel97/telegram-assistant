...
async def start_pc_via_wol(update: Update, context: CallbackContext):
	if is_not_correct_chat_id(update.message.chat_id):
		...
		return
	start_without_monitors = context.args[0] != "False" if context.args else True

	requests.post("http://192.168.178.110:8123/api/webhook/" + WOL_WEBHOOK, timeout=30)
	if start_without_monitors:
		requests.post("http://tasker-proxy.custom.svc.cluster.local:9000/wol/status", timeout=30)
