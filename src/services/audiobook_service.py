...
def update_audiobook_statistics(new_records):
	...
	done_message = f"added {len(to_add)} / updated {len(to_update)} / unchanged {len(unchanged)} - from {len(new_records)} in total"
...
def read_audiobook_statistics(path):
	xml_data = open(path, encoding="utf-8").read()  # Read file
