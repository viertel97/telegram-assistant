...
def video_to_text(update: Update, context: CallbackContext):
    ...
    recognized_text = r.recognize_google(audio, language=VIDEO_TRANSCRIBE_LANGUAGE)
    now = datetime.now()
    message = '"{recognized_text}" on {date} at {time}'.format(
        recognized_text=recognized_text, date=now.strftime("%d.%m.%Y"), time=now.strftime("%H:%M")
    )
